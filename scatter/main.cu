...
__global__ void
s2g_gpu_scatter_kernel(uint32_t *in, uint32_t *out, int len)
{
    {
...
static void
s2g_cpu_scatter(uint32_t *in, uint32_t *out, int len)
{
...
static void
s2g_gpu_scatter(uint32_t *in, uint32_t *out, int len)
{
    // Invoke kernel
    int threadsPerBlock = 256; // Arbitrary number?

    int blocksPerGrid = (len + threadsPerBlock - 1) / threadsPerBlock;
    s2g_gpu_scatter_kernel<<<blocksPerGrid, threadsPerBlock>>>(in, out, len);
}
...
eval(int inputLength)
{
    ...
    const std::string conf_info = std::string("scatter[len:") + std::to_string(inputLength) + "]";
