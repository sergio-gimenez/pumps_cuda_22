...
void oss_potrf(double * const A, int ts, int ld)
{
   static int INFO;
...
void oss_trsm(double *A, double *B, int ts, int ld)
{
...
#pragma oss task in([ts][ts]A) inout([ts][ts]B)
void oss_syrk(double *A, double *B, int ts, int ld)
{
...
void oss_gemm(double *A, double *B, double *C, int ts, int ld)
{
...
int main(int argc, char* argv[])
{
   ...
   printf( "==========================================\n" );
#else
   printf( "test:%s-%d-%d:threads:%2d:result:%s:gflops:%f\n", argv[0], n, ts, oss_get_num_threads(), result[check], gflops );
#endif

   // Free blocked matrix memory
   for (int i = 0; i < nt; i++) {
      for (int j = 0; j < nt; j++) {
         assert(Ah[i][j] != NULL);
         free(Ah[i][j]);
