...
inline void sendFirstComputeRow(block_t *matrix, int nbx, int nby, int rank, int rank_size)
{
	for (int by = 1; by < nby-1; ++by) {
		#pragma oss task label("send first compute row") in(([nbx][nby]matrix)[1][by])
...
{
	for (int by = 1; by < nby-1; ++by) {
		#pragma oss task label("send last compute row") in(([nbx][nby]matrix)[nbx-2][by])
...
{
	for (int by = 1; by < nby-1; ++by) {
		#pragma oss task label("receive upper border") out(([nbx][nby]matrix)[0][by])
...
{
	for (int by = 1; by < nby-1; ++by) {
		#pragma oss task label("receive lower border") out(([nbx][nby]matrix)[nbx-1][by])
