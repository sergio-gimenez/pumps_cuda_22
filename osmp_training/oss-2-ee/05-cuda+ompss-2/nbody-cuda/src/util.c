...
void Particle_pair_check (Particle* this_particle1, Particle* this_particle2, char* action, char* routine)
{ 
...
#ifdef CHECK
void Particle_array_check ( Particle* this_particle_array, int number_of_particles, char* action, char* routine)
{
...
Particle* Particle_array_allocate (int number_of_particles)
{
	...
	cudaMallocManaged( (void **) &this_particle_array, sizeof(Particle) * number_of_particles, cudaMemAttachGlobal);
#endif
...
#ifdef CHECK
	Particle_array_check(this_particle_array, number_of_particles, "initialize randomly", "Particle_array_initialize_randomly");
#endif
